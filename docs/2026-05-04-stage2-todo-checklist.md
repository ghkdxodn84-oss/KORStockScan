...
- [ ] `[OpenAIResponsesWS0504-Intraday] OpenAI Responses WS shadow queue/timeout/fallback 1차 판정` (`Due: 2026-05-04`, `Slot: INTRADAY`, `TimeWindow: 10:00~10:20`, `Track: ScalpingLogic`)
  ...
  - 다음 액션: `http fallback<=2%`, `parse_fail<=0.5%`, `timeout_reject_rate<=1%`가 아니면 POSTCLOSE에 shadow 유지/원인분해만 남기고, 런타임 미사용이면 `AIEngineFlagOffBacklog0506`에서 `OpenAI live routing backlog only`로 재분류한다.
