...
- 물타기/불타기 `MAX_*_COUNT`는 더 이상 runtime blocker가 아니라 attribution counter다. generic `AVG_DOWN` 평가는 제거했고, scalping `AVG_DOWN` add_type은 `REVERSAL_ADD` 귀속명으로만 남긴다. 반복 추가매수 리스크는 `REVERSAL_ADD`/`PYRAMID` enable, cooldown, pending order, position cap, protection 재설정, near-close gate로 제한한다.
