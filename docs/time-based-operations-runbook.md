...
| `09:05~15:30` | cron | `deploy/run_panic_sell_defense_intraday.sh` 2분 주기, 5분 배수 분 제외 offset, wrapper cooldown 90초 | `data/report/panic_sell_defense/panic_sell_defense_YYYY-MM-DD.{json,md}`, `logs/run_panic_sell_defense_cron.log` | `panic_state`, stop-loss cluster, active sim/probe 회복률, post-sell rebound, `canary_candidates`, `runtime_effect=report_only_no_mutation`, CPU/resource spike 반복 여부 확인 | panic 결과로 score/stop threshold 변경, 자동매도, bot restart, 스윙 실주문 전환 금지. 2분 전환 후 resource fail이 반복되면 5분 주기로 rollback |
