...
12. OFI/QI source-quality는 `stale_missing_flag` 단일 boolean으로만 보지 않고 `micro_missing`, `micro_stale`, `observer_unhealthy`, `micro_not_ready`, `state_insufficient` reason과 unique record count를 함께 본다. 이 값은 `source_quality_blocked_families`와 approval/workorder blocker 입력으로 쓸 수 있지만, 단독 runtime mutation 근거는 아니다.
