...
22. `20:05` tuning monitoring은 `threshold_cycle_postclose`의 같은 날짜 최신 terminal marker가 `[DONE]`이 아니면 대기하거나 fail-closed한다. postclose가 장시간화되어 20:05 이후에도 실행 중이면 Parquet/DuckDB refresh가 선행 산출물을 덜 소비하지 않도록 wrapper dependency를 우선한다.
23. 날짜별 checklist를 수정했다면 parser 검증 후 Project/Calendar 동기화 명령을 사용자에게 남긴다.
24. OpenAI AI correction은 품질 우선 `gpt-5.5` 경로라 수 분 단위로 걸릴 수 있다. 15분 이내 실행 중이면 `not_yet_due`, 15분 초과 미생성이면 cron log와 job 종료 여부를 확인해 `warning` 또는 `fail`로 분류한다. cron timeout은 이보다 짧게 잡지 않는다.
