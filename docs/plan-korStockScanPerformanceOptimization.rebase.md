...
| 진입가 | reference target이 실주문가/timeout을 과도하게 왜곡하는 문제와 submitted 직전 불량 호가 흐름 진입 | 미체결 기회비용과 불리한 추격진입이 동시에 생길 수 있다 | `dynamic_entry_price_resolver_p1`, `dynamic_entry_ai_price_canary_p2`는 [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md)가 로드/health check를 소유한다. OFI/QI P2 내부 feature는 `OrderbookMicroP2Canary0504-Postclose`가 micro-enabled P2 cohort의 SKIP/fill/soft stop/missed upside를 판정하고, [2026-05-08 checklist](./2026-05-08-stage2-todo-checklist.md) `OFIQExpansionLadder0508`이 prompt/report/calibration 확장계획과 stale 방지 owner를 소유한다 |
...
| --- | --- | --- | --- |
| entry operating override | `mechanical_momentum_latency_relief` ON. `2026-05-04`에는 신규 OPEN 작업이 아니라 `RuntimeFlagInventory0504-Preopen`에서 로드/상태만 재확인한다 | [2026-04-30 checklist](./2026-04-30-stage2-todo-checklist.md) `MechanicalMomentumLatencyRelief0430-*`, [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `RuntimeFlagInventory0504-Preopen` | 반영 완료. 현재 OPEN owner 누락 없음 |
...
| holding/exit live canary | `soft_stop_micro_grace`, `REVERSAL_ADD`, `bad_entry_refined_canary`가 현재 보유/청산 live owner다. `same_symbol_loss_reentry_cooldown`은 유안타증권 반복손실 대응으로 연 임시 운영가드이며, 최종 threshold 축이 아니다. `2026-05-04`는 refined bad-entry keep/OFF와 flow override 분포를 먼저 닫고, `REVERSAL_ADD`/`PYRAMID` 동적 수량 및 손실 후 하향 재진입 복합 threshold는 `2026-05-06` observe/report-only 설계 owner로 분리한다. `protect_trailing_stop`은 단일 tick 이탈이 아니라 rolling price smoothing으로 확정하며, `protect_hard_stop`은 평탄화 대상이 아니다 | [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `BadEntryRefinedCanary0504-*`, `ProtectTrailingSmoothing0504-Intraday`, `SameSymbolLossReentryCooldown0504-Intraday`, `YuantaDowntrendReentry0504-Postclose`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `ReversalAddDynamicQty0506`, `PyramidDynamicQty0506`, `CooldownPolicyInventory0506`, `DowntrendReentryComposite0506` | `2026-05-04` 리노공업/한화비전 보호트레일링 이탈 대응으로 단일 tick 민감도 평탄화 hotfix를 반영했고, 유안타증권 반복손실 대응으로 손실 청산 후 동일종목 60분 재진입 cooldown을 임시로 열었다. `hard_time_stop_shadow`, `same_symbol_soft_stop_cooldown_shadow`, `partial_only_timeout_shadow`는 runtime 기본 OFF로 내렸고, 단독 cooldown은 5/6 복합 threshold 전환 후보로만 본다. |
