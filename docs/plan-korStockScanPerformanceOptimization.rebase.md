...
| holding/exit live canary | `soft_stop_micro_grace`, `REVERSAL_ADD`, `holding_flow_override`가 현재 보유/청산 live owner다. OFI smoother는 `holding_flow_override` 내부에서만 `EXIT` debounce와 bearish confirm으로 쓰며 hard/protect/order safety, `max_defer_sec`, `worsen_floor`를 넘지 않는다. `bad_entry_refined_canary`는 5/4 장후 OFF, refined 후보는 `SCALP_BAD_ENTRY_REFINED_OBSERVE_ENABLED=True` report-only counterfactual로만 유지한다. `same_symbol_loss_reentry_cooldown`은 유안타증권 반복손실 대응으로 연 임시 운영가드이며 최종 threshold 축이 아니다. `REVERSAL_ADD`/`PYRAMID` 동적 수량, trailing/protect 민감도, winner wide-window 전환 후보, 손실 후 하향 재진입 복합 threshold는 `2026-05-06` observe/report-only 설계 owner로 분리한다. `protect_trailing_stop`은 단일 tick 이탈이 아니라 rolling price smoothing으로 확정하며, `protect_hard_stop`은 평탄화 대상이 아니다 | [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `BadEntryRefinedCanary0504-Postclose`, `HoldingFlowOverride0504-*`, `ProtectTrailingSmoothing0504-Intraday`, `TrailingProfitSensitivity0504-Postclose`, `SameSymbolLossReentryCooldown0504-Intraday`, `YuantaDowntrendReentry0504-Postclose`, `KAIWideWindowExit0504-Postclose`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `OFIAISmoothingOverride0506-Preopen`, `ThresholdOpsTransition0506`, `BadEntryRefinedRollback0506-Preopen`, `OvernightFlowTrimSemantics0506-Preopen`, `ReversalAddDynamicQty0506`, `PyramidDynamicQty0506`, `SwingTrailingPolicy0506`, `TrailingProtectSensitivity0506`, `CooldownPolicyInventory0506`, `DowntrendReentryComposite0506` | `2026-05-04` 리노공업/한화비전 보호트레일링 이탈 대응으로 단일 tick 민감도 평탄화 hotfix를 반영했고, 유안타증권 반복손실 대응으로 손실 청산 후 동일종목 60분 재진입 cooldown을 임시로 열었다. `bad_entry_refined_canary`는 flow defer/sell failure 혼입으로 OFF, 다음 축은 report-only counterfactual enrichment로 즉시 전환했다. trailing/protect 민감도는 5/4 수익 실현은 양호하나 weak borderline, winner wide-window 후보, PYRAMID 교차가 커서 5/6 단일 owner로 재판정한다. 오버나이트 `TRIM`은 `SELL_TODAY` 유지로 보정했고, 장마감 후 SCALPING 매도 신호는 `sell_order_blocked_market_closed`로 주문 전송을 막는다. |
| position sizing / scale-in semantics | 신규 BUY `1주 cap`은 유지하고, `MAX_*_COUNT`는 runtime blocker가 아니라 attribution counter로만 본다. generic `AVG_DOWN` 평가는 제거했고, scalping `AVG_DOWN` add_type은 `REVERSAL_ADD` 귀속명으로만 남긴다. 추가매수 리스크 제한은 `REVERSAL_ADD`/`SCALPING_ENABLE_PYRAMID`와 fill cooldown/cancel cooldown/pending/position cap/protection 기반 가드가 맡고, 수량 자체의 EV 재설계는 `2026-05-06` observe-only owner로 분리한다 | [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `ScaleInCountGateRemoval0504-Preopen`; [workorder-shadow-canary-runtime-classification](./workorder-shadow-canary-runtime-classification.md) `initial_entry_qty_cap_1share`, `SCALPING_ENABLE_PYRAMID`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `ReversalAddDynamicQty0506`, `PyramidDynamicQty0506` | `2026-05-04` `456040` PYRAMID 접수/취소 반복 대응으로 pending add 취소 성공 후 `SCALE_IN_CANCEL_COOLDOWN_SEC=120` 재접수 가드를 추가했다. 현재 OPEN owner 누락 없음 |
