...
| holding/exit live canary | `soft_stop_micro_grace`, `REVERSAL_ADD`, `holding_flow_override`가 현재 보유/청산 live owner다. `bad_entry_refined_canary`는 5/4 장후 OFF, refined 후보는 `SCALP_BAD_ENTRY_REFINED_OBSERVE_ENABLED=True` report-only counterfactual로만 유지한다. `same_symbol_loss_reentry_cooldown`은 유안타증권 반복손실 대응으로 연 임시 운영가드이며 최종 threshold 축이 아니다. `REVERSAL_ADD`/`PYRAMID` 동적 수량, trailing/protect 민감도, 손실 후 하향 재진입 복합 threshold는 `2026-05-06` observe/report-only 설계 owner로 분리한다. `protect_trailing_stop`은 단일 tick 이탈이 아니라 rolling price smoothing으로 확정하며, `protect_hard_stop`은 평탄화 대상이 아니다 | [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `BadEntryRefinedCanary0504-Postclose`, `HoldingFlowOverride0504-*`, `ProtectTrailingSmoothing0504-Intraday`, `TrailingProfitSensitivity0504-Postclose`, `SameSymbolLossReentryCooldown0504-Intraday`, `YuantaDowntrendReentry0504-Postclose`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `BadEntryRefinedRollback0506-Preopen`, `OvernightFlowTrimSemantics0506-Preopen`, `ReversalAddDynamicQty0506`, `PyramidDynamicQty0506`, `TrailingProtectSensitivity0506`, `CooldownPolicyInventory0506`, `DowntrendReentryComposite0506` | `2026-05-04` 리노공업/한화비전 보호트레일링 이탈 대응으로 단일 tick 민감도 평탄화 hotfix를 반영했고, 유안타증권 반복손실 대응으로 손실 청산 후 동일종목 60분 재진입 cooldown을 임시로 열었다. `bad_entry_refined_canary`는 flow defer/sell failure 혼입으로 OFF, 다음 축은 report-only counterfactual enrichment로 즉시 전환했다. trailing/protect 민감도는 5/4 수익 실현은 양호하나 weak borderline과 PYRAMID 교차가 커서 5/6 단일 owner로 재판정한다. 오버나이트 `TRIM`은 `SELL_TODAY` 유지로 보정했고, 장마감 후 SCALPING 매도 신호는 `sell_order_blocked_market_closed`로 주문 전송을 막는다. |
