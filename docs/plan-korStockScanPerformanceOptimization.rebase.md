...
| position sizing / scale-in semantics | 신규 BUY `1주 cap`은 유지하고, `MAX_*_COUNT`는 runtime blocker가 아니라 attribution counter로만 본다. generic `AVG_DOWN` 평가는 제거했고, scalping `AVG_DOWN` add_type은 `REVERSAL_ADD` 귀속명으로만 남긴다. 추가매수 리스크 제한은 `REVERSAL_ADD`/`SCALPING_ENABLE_PYRAMID`와 cooldown/pending/position cap/protection 기반 가드가 맡고, 수량 자체의 EV 재설계는 `2026-05-06` observe-only owner로 분리한다 | [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `ScaleInCountGateRemoval0504-Preopen`; [workorder-shadow-canary-runtime-classification](./workorder-shadow-canary-runtime-classification.md) `initial_entry_qty_cap_1share`, `SCALPING_ENABLE_PYRAMID`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `ReversalAddDynamicQty0506`, `PyramidDynamicQty0506` | 기존 요약표 누락분 반영 완료. 현재 OPEN owner 누락 없음 |
