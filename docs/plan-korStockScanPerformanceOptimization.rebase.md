...
| holding/exit live canary | `soft_stop_micro_grace`, `REVERSAL_ADD`, `bad_entry_refined_canary`가 현재 보유/청산 live owner다. `2026-05-04`는 refined bad-entry keep/OFF와 flow override 분포를 먼저 닫고, `REVERSAL_ADD`/`PYRAMID` 동적 수량은 `2026-05-06` observe-only 설계 owner로 분리한다. `protect_trailing_stop`은 단일 tick 이탈이 아니라 rolling price smoothing으로 확정하며, `protect_hard_stop`은 평탄화 대상이 아니다 | [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `BadEntryRefinedCanary0504-*`, `ProtectTrailingSmoothing0504-Intraday`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `ReversalAddDynamicQty0506`, `PyramidDynamicQty0506` | `2026-05-04` 리노공업/한화비전 보호트레일링 이탈 대응으로 단일 tick 민감도 평탄화 hotfix 반영. 평탄화 파라미터는 `protect_trailing_smoothing` threshold family로 수집하되, `ThresholdOpsTransition0506` 전에는 `manifest_only` 추천값만 생성하고 live auto-mutation은 금지한다. 현재 OPEN owner 누락 없음 |
