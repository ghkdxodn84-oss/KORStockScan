...
| `bad_entry_block` | soft stop으로 이어질 가능성이 큰 never-green/AI fade 유형을 조기정리 후보로 분류하는 observe-only classifier | `2026-04-30` observe-only. 표본 수는 충분하지만 `GOOD_EXIT` 제거 위험이 남아 단순 live block은 금지한다. `bad_entry_refined_canary`는 `2026-05-04` 장후 canary-applied cohort가 flow defer와 장마감 `sell_order_failed` 반복에 섞여 rollback guard로 OFF됐다. 재개는 기존 keep이 아니라 새 단일축 workorder와 rollback guard가 필요하다 |
...
6. 원인 귀속이 불명확하면 전략 threshold를 먼저 바꾸지 않고 리포트 정합성, 이벤트 복원, 집계 품질을 먼저 점검한다.
...
1. live 변경은 동일 단계 내 1축 canary를 기본으로 한다. 복합 entry canary는 단일축 원칙의 예외가 아니라 `단일 가설을 구성하는 묶음 축`이며, 묶음 파라미터는 개별 ON/OFF 효과로 분리 판정하지 않는다.
...
| `initial_entry_qty_cap` | prompt 재교정 이후 신규 BUY 변동성이 커질 때 스캘핑 초기 진입 수량을 제한하는 운영가드 | entry/holding/exit 관찰 기간 | `2026-04-30` 장후 사용자 지시로 최대매수가능 주수는 `1주 cap`으로 회귀한다. 초기 진입 risk tail과 holding/exit 원인귀속 보존을 우선한다. `PYRAMID zero_qty`는 `2026-05-04` 사용자 지시로 신규 진입 cap 확대가 아니라 스캘핑 PYRAMID 1주 floor bugfix로 처리한다. |
...
| --- | --- | --- | --- |
| 진입 | BUY 후 제출 전 병목과 mechanical fallback score 표본 처리 | 제출 drought가 지속되면 보유/청산 개선 표본도 고갈된다 | `mechanical_momentum_latency_relief`는 [2026-04-30 checklist](./2026-04-30-stage2-todo-checklist.md) `MechanicalMomentumLatencyRelief0430-*`에서 반영/판정 완료. 현재는 신규 OPEN 작업이 아니라 운영 override ON 상태다. 다만 `AI score 50`은 [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `AIScore50BuyHoldOverride0504-Intraday` 이후 매수보류 override가 우선한다 |
| 진입가 | reference target이 실주문가/timeout을 과도하게 왜곡하는 문제와 submitted 직전 불량 호가 흐름 진입 | 미체결 기회비용과 불리한 추격진입이 동시에 생길 수 있다 | `dynamic_entry_price_resolver_p1`, `dynamic_entry_ai_price_canary_p2`는 [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md)가 로드/health check를 소유한다. OFI/QI P2 내부 feature는 `OrderbookMicroP2Canary0504-Postclose`에서 provenance와 global-threshold fallback을 확인했고, low-confidence SKIP demotion은 P2 내부 운영 override로만 본다. [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `OFIAISmoothingOverride0506-Preopen`이 로드를 소유하고 `ThresholdOpsTransition0506`의 OFI smoothing integration 메모가 manifest_only family 판정을 소유하며, [2026-05-08 checklist](./2026-05-08-stage2-todo-checklist.md) `OFIQExpansionLadder0508`은 prompt/report/calibration 확장계획을 소유한다 |
...
| 보유/청산 | soft stop tail, never-green/AI fade, 조급한 전량청산, winner를 너무 짧은 스캘핑 호흡으로 닫은 뒤 더 높은 가격에 재진입하는 churn | 손실 축소보다 missed upside 회복과 bad-entry tail 절단의 순EV가 핵심이다 | `soft_stop_micro_grace`, `REVERSAL_ADD`, `holding_flow_override`를 분리. OFI smoother는 `holding_flow_override` 내부 postprocessor이며 standalone EXIT gate가 아니다. `bad_entry_refined_canary`는 [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `BadEntryRefinedCanary0504-Postclose`에서 OFF됐다. 다음 축은 같은 checklist에서 `bad-entry report-only counterfactual enrichment`로 조기 판정했고, [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `BadEntryRefinedRollback0506-Preopen`이 로드 확인만 소유한다. `한국항공우주` `ID 4932` 같은 winner wide-window 후보는 [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `SwingTrailingPolicy0506`/`TrailingProtectSensitivity0506`에서 report-only 전환 조건으로만 보며, 3/5/10분 wide OFI persistence는 보조 feature일 뿐 live swing 승격 gate가 아니다 |
...
| 의사결정 지원 | 가격대/거래량/시간대별 행동가중치, AI decision matrix, 15:00 preclose sell target이 runtime 판단과 분리됨 | 청산/물타기/불타기/오버나이트/스윙 전환 선택의 기회비용을 놓치면 기대값 개선축 선정이 늦어진다 | `statistical_action_weight`, `stat_action_decision_snapshot`, `holding_exit_decision_matrix`는 [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `StatActionDecisionSnapshot0504-Preopen`, [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `StatActionWeight0506`, `StatActionMarkdown0506`, `StatActionAdvancedAxes0506`, `AIDecisionMatrix0506`, [2026-05-07 checklist](./2026-05-07-stage2-todo-checklist.md) `StatActionEligibleOutcome0507`, `AIDecisionMatrixShadow0507`, [2026-05-08 checklist](./2026-05-08-stage2-todo-checklist.md) `StatActionAdvancedContext0508`가 report-only/ADM ladder로 소유한다. `preclose_sell_target`은 [preclose-sell-target-revival-plan](./preclose-sell-target-revival-plan.md)과 [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `PrecloseSellTargetRevival0506-Intraday`가 report-only 재개를 소유한다 |
| 분석 품질 | missed-entry counterfactual가 아직 `optimistic/realistic/conservative` 3모드로 분리되지 않았고, 청산 authority provenance도 field taxonomy가 잠기지 않음 | 기회비용 해석과 청산 원인귀속이 계속 단일 수치/암묵 규칙에 의존하면 기대값 개선축 우선순위가 다시 흐려진다 | [2026-05-08 checklist](./2026-05-08-stage2-todo-checklist.md) `Phase3Quality0508`이 `3-mode counterfactual`과 `exit_decision_source provenance`의 채택/미채택을 분리 판정한다 |
...
| --- | --- | --- | --- |
| entry operating override | `mechanical_momentum_latency_relief` ON. `AI_SCORE_50_BUY_HOLD_OVERRIDE_ENABLED=True`가 score 50 fallback/neutral 신규 BUY를 `blocked_ai_score`로 보류한다. `2026-05-06` `entry_armed` 급감 재분해에서 score 50뿐 아니라 `WAIT 65~74`/`ai_score 65~74 blocked_ai_score`가 budget/pass 이전 병목으로 확인됐으므로, missed-winner/avoided-loser 장후 판정 범위에 `wait65_74_*` report-only 후보를 포함한다 | [2026-04-30 checklist](./2026-04-30-stage2-todo-checklist.md) `MechanicalMomentumLatencyRelief0430-*`, [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `RuntimeFlagInventory0504-Preopen`, `AIScore50BuyHoldOverride0504-Intraday`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `Score50BlockedTriage0506-Intraday`, `CooldownPolicyInventory0506`, `DowntrendReentryComposite0506` | score 50 fallback은 유안타증권 반복손실 대응으로 신규 BUY 보류 override가 우선한다. 5/6에는 hard cooldown 단독 blocker 목록과 score50/wait65_74 missed winner/avoided loser, 하향 재진입 복합 threshold 대체 가능성을 분리 판정한다 |
| entry price baseline/live | `dynamic_entry_price_resolver_p1` baseline 경로 + `dynamic_entry_ai_price_canary_p2` active canary. P2 실패는 P1로 fail-closed. OFI/QI orderbook micro는 P2 내부 입력으로만 쓰며 신규 standalone entry canary로 세지 않는다. confidence `80~89` raw `SKIP` 중 non-bearish OFI만 `entry_ai_price_ofi_skip_demoted`로 P1 defensive 경로에 남긴다. `2026-05-06`에는 latency entry/exit price guard 재튜닝, `80bps` 분포 재앵커, 비-SCALPING observe-only guard, `submitted_order_price` canonical 승격, resolver ingress gate를 carry-over follow-up으로 닫는다 | [2026-05-02 checklist](./2026-05-02-stage2-todo-checklist.md) `OrderbookMicroP2Canary0502-DesignApply`, [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `DynamicEntryResolverP10504-*`, `DynamicEntryAIPriceCanary0504-*`, `OrderbookMicroP2Canary0504-Postclose`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `OFIAISmoothingOverride0506-Preopen`, `ThresholdOpsTransition0506`, `LatencyEntryPriceGuard0506HolidayCarry`, `LatencyEntryPriceGuardV2_0506HolidayCarry`, `LatencyExitPriceGuardReview0506HolidayCarry`, `PreSubmitGuardDist0506HolidayCarry`, `PreSubmitGuardObserve0506HolidayCarry`, `BuyPriceSchemaSplitP1_0506HolidayCarry`, `DynamicEntryResolverIngress0506HolidayCarry`; [2026-05-08 checklist](./2026-05-08-stage2-todo-checklist.md) `OFIQExpansionLadder0508` | `2026-05-05` 휴장일 OPEN 항목은 모두 `2026-05-06`으로 이관됐다. entry price follow-up OPEN owner 누락 없음 |
| AI engine: Gemini | P0 JSON fast-path는 반영됨. P1 `system_instruction`, P1 deterministic JSON config, P2 schema registry는 flag-off 준비/관찰 승인이고 live enable 미승인. `2026-05-06` 삼성생명 보유감시 표본에서 `ai_holding_review` cache MISS 100%와 `holding_flow_override_review` Tier2 provenance 필드 누락이 확인되어, [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `HoldingAICacheAndTierLogging0506-Intraday`가 로깅 보강과 `15:10 KST` 1차 / `17:45 KST` 최종 판정을 소유한다 | [workorder_gemini_engine_review](./workorder_gemini_engine_review.md), [2026-04-29 Gemini spec](./2026-04-29-gemini-enable-acceptance-spec.md), [2026-04-30 checklist](./2026-04-30-stage2-todo-checklist.md) `GeminiSchemaIngress0430`, `GeminiSchemaContractCarry0430`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `AIEngineFlagOffBacklog0506`, `HoldingAICacheAndTierLogging0506-Intraday` | 5/6 재점검 항목으로 보강 완료. cache/Tier provenance는 observe-only 로깅 보강이며, cache TTL/model routing 변경은 별도 checklist 없이는 열지 않는다 |
...
| holding/exit live canary | `soft_stop_micro_grace`, `REVERSAL_ADD`, `holding_flow_override`가 현재 보유/청산 live owner다. OFI smoother는 `holding_flow_override` 내부에서만 `EXIT` debounce와 bearish confirm으로 쓰며 hard/protect/order safety, `max_defer_sec`, `worsen_floor`를 넘지 않는다. `bad_entry_refined_canary`는 5/4 장후 OFF, refined 후보는 `SCALP_BAD_ENTRY_REFINED_OBSERVE_ENABLED=True` report-only counterfactual로만 유지한다. `same_symbol_loss_reentry_cooldown`은 유안타증권 반복손실 대응으로 연 임시 운영가드이며 최종 threshold 축이 아니다. `2026-05-06`에는 REVERSAL_ADD/PYRAMID의 신호 조건 대규모 재튜닝은 하지 않고 주문 직전 scale-in P1 가격 resolver와 0/1 동적 수량 safety만 live로 교체했다. trailing/protect 민감도, winner wide-window 전환 후보, 손실 후 하향 재진입 복합 threshold는 observe/report-only 설계 owner로 분리한다. `protect_trailing_stop`은 단일 tick 이탈이 아니라 rolling price smoothing으로 확정하며, `protect_hard_stop`은 평탄화 대상이 아니다 | [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `BadEntryRefinedCanary0504-Postclose`, `HoldingFlowOverride0504-*`, `ProtectTrailingSmoothing0504-Intraday`, `TrailingProfitSensitivity0504-Postclose`, `SameSymbolLossReentryCooldown0504-Intraday`, `YuantaDowntrendReentry0504-Postclose`, `KAIWideWindowExit0504-Postclose`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `OFIAISmoothingOverride0506-Preopen`, `ThresholdOpsTransition0506`, `BadEntryRefinedRollback0506-Preopen`, `OvernightFlowTrimSemantics0506-Preopen`, `ScaleInPriceResolverP1_0506`, `ScaleInDynamicQty0506`, `SwingTrailingPolicy0506`, `TrailingProtectSensitivity0506`, `CooldownPolicyInventory0506`, `DowntrendReentryComposite0506` | `2026-05-04` 리노공업/한화비전 보호트레일링 이탈 대응으로 단일 tick 민감도 평탄화 hotfix를 반영했고, 유안타증권 반복손실 대응으로 손실 청산 후 동일종목 60분 재진입 cooldown을 임시로 열었다. `bad_entry_refined_canary`는 flow defer/sell failure 혼입으로 OFF, 다음 축은 report-only counterfactual enrichment로 즉시 전환했다. trailing/protect 민감도는 5/4 수익 실현은 양호하나 weak borderline, winner wide-window 후보, PYRAMID 교차가 커서 5/6 단일 owner로 재판정한다. 오버나이트 `TRIM`은 `SELL_TODAY` 유지로 보정했고, 장마감 후 SCALPING 매도 신호는 `sell_order_blocked_market_closed`로 주문 전송을 막는다. |
| position sizing / scale-in semantics | 신규 BUY `1주 cap`은 유지하고, `MAX_*_COUNT`는 runtime blocker가 아니라 attribution counter로만 본다. generic `AVG_DOWN` 평가는 제거했고, scalping `AVG_DOWN` add_type은 `REVERSAL_ADD` 귀속명으로만 남긴다. 추가매수 리스크 제한은 `REVERSAL_ADD`/`SCALPING_ENABLE_PYRAMID`와 fill cooldown/cancel cooldown/pending/position cap/protection 기반 가드가 맡는다. `2026-05-06` 삼성생명 PYRAMID 추격 지정가 표본 이후, PYRAMID 임시 가격가드는 scale-in P1 resolver hard safety로 흡수했고 `SCALPING_SCALE_IN_PRICE_RESOLVER_ENABLED=True`, `SCALPING_SCALE_IN_DYNAMIC_QTY_ENABLED=True`, `SCALPING_SCALE_IN_EFFECTIVE_QTY_CAP=1`, spread `80bp`, PYRAMID AI/수급/가속 threshold `70/60/0.5`로 REVERSAL_ADD/PYRAMID 주문 직전 가격·수량 경로를 교체했다. P2 `scale_in_price_v1`은 observe-only다 | [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `ScaleInCountGateRemoval0504-Preopen`; [workorder-shadow-canary-runtime-classification](./workorder-shadow-canary-runtime-classification.md) `initial_entry_qty_cap_1share`, `SCALPING_ENABLE_PYRAMID`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `SamsungLifePyramidPriceGuard0506-Intraday`, `ScaleInPriceResolverP1_0506`, `ScaleInDynamicQty0506` | `2026-05-04` `456040` PYRAMID 접수/취소 반복 대응으로 pending add 취소 성공 후 `SCALE_IN_CANCEL_COOLDOWN_SEC=120` 재접수 가드를 추가했다. 삼성생명 id `5132`에서 PYRAMID 직전 `spread_bps 83.19/82.64`, `distance_to_buy_bps 203.74/176.62`, `tick_acceleration_ratio 0.0/0.0` 상태로 `ws_data.curr` 지정가가 전송된 것을 가격품질 결함으로 보고, 다음 scale-in 주문부터 `request_price == ws_data.curr` 직접 제출 재발을 rollback guard로 본다 |
| holding/overnight operating override | `holding_flow_override`는 튜닝 관찰축이 아니라 운영 override. hard/protect/order safety는 우회하지 않는다. 5/4 장후 `TRIM -> HOLD_OVERNIGHT` semantics는 폐기했고, flow `HOLD`만 오버나이트 보유로 승격한다 | [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `HoldingFlowOverride0504-*`, [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `OvernightFlowTrimSemantics0506-Preopen`, [2026-05-01 review](./2026-05-01-holding-flow-override-code-review-report.md) | 운영 override 자체는 ON/반영 완료지만, 5/6 PREOPEN의 `TRIM=SELL_TODAY` 로드 확인은 OPEN이다 |
| threshold cycle / action weight | threshold compact collector, 장후 daily report, 누적/rolling cohort report, 다음 장전 manifest/apply, `statistical_action_weight`, `preclose_sell_target`는 runtime mutation 전 단계다. 현재 live threshold 자동변경은 금지하고 `manifest_only`로 둔다. `entry_ofi_ai_smoothing`, `holding_flow_ofi_smoothing`, `scale_in_price_guard` family는 dynamic threshold 후보를 산출하더라도 `ThresholdOpsTransition0506` 전에는 manifest_only 후보만 만든다. `scale_in_price_guard`는 `scale_in_price_resolved`/`scale_in_price_guard_block`/`scale_in_price_p2_observe`를 수집해 spread, micro-VWAP, PYRAMID AI/수급/가속, effective qty cap의 분포와 block reason을 장후 report 입력으로 남긴다. 누적/rolling report는 `daily`, `rolling`, `cumulative` 방향성 일치 여부를 보는 report-only 입력이며, 누적 평균 단독으로 threshold를 자동 적용하지 않는다. 자동 threshold 적용, bot restart, version attribution의 추적성은 [report-based-automation-traceability](./report-based-automation-traceability.md)의 `R0~R6` ladder로 관리한다 | [2026-05-01 checklist](./2026-05-01-stage2-todo-checklist.md) `ThresholdBootstrap0501-AM`, `StatActionMatrixReport0501-Maintenance`; [2026-05-04 checklist](./2026-05-04-stage2-todo-checklist.md) `CumulativeThresholdCycleReport0504-Postclose`; [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `OFIAISmoothingOverride0506-Preopen`, `ScaleInPriceGuardThresholdCycle0506-Intraday`, `ThresholdCollectorIO0506`, `ReportAutomationTraceability0506`, `ThresholdOpsTransition0506`, `StatActionWeight0506`, `StatActionMarkdown0506`, `StatActionAdvancedAxes0506`, `PrecloseSellTargetRevival0506-Intraday`; [2026-05-07 checklist](./2026-05-07-stage2-todo-checklist.md) `StatActionEligibleOutcome0507`; [2026-05-08 checklist](./2026-05-08-stage2-todo-checklist.md) `StatActionAdvancedContext0508` | 누적/rolling 산출물은 report-only로 반영 완료. OFI smoothing과 scale-in price guard family는 sample floor와 daily/rolling 방향 일치를 만족해도 runtime env/code를 자동 변경하지 않는다. `preclose_sell_target`은 canonical JSON/Markdown 재개 준비까지 완료했고 5/6 15:00 dry-run으로 정기화 여부를 판정한다. R5/R6는 5/6 acceptance 전까지 blocked/pending으로 추적 |
...
| counterfactual / exit authority provenance | `missed_entry_counterfactual` 리포트는 운영 중이지만 3모드 realism은 아직 없고, `exit_decision_source` taxonomy는 runtime/report owner가 비어 있다 | [2026-05-08 checklist](./2026-05-08-stage2-todo-checklist.md) `Phase3Quality0508` | 부분 채택 상태. 5/8에 채택/미채택과 후속 owner를 분리 판정 |
...
| runtime stabilization | `nan_cast_guard_followup`, execution receipt binding/thread safety, `holding_state net-profit sell decision` 회귀 격리는 EV 판정 전제 품질축이다. threshold IO/ops는 위 `threshold cycle / action weight` 행에서 별도 관리한다 | [2026-05-06 checklist](./2026-05-06-stage2-todo-checklist.md) `NaNCastGuard0506HolidayCarry`, `ExecutionReceiptsThreadSafety0506`, `HoldingNetProfitExitRegression0506` | `2026-05-05` 휴장일 OPEN 누락 없음. `HoldingNetProfitExitRegression0506`까지 반영 완료. `2026-05-04` 지투파워 `ID 4988` 0주 매도가능/중복 completed는 `ExecutionReceiptsThreadSafety0506`의 receipt binding anchor로 추가됐다 |

종료/폐기 축과 historical 품질 gate(`ShadowDiff0428` 등)는 이 표에서 제외하고 [closed observation archive](./archive/closed-observation-axes-2026-05-01.md) 또는 관련 Q&A/체크리스트에서만 참조한다. 현재 OPEN 상태 기준으로 별도 누락된 owner는 없다.
