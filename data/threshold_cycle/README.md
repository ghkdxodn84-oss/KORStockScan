...
| `data/threshold_cycle/approvals/swing_one_share_real_canary_YYYY-MM-DD.json` | 스윙 초기 BUY/SELL 1주 real canary 전용 approval artifact. 승인 종목 allowlist와 cap이 있어야만 `SWING_ONE_SHARE_REAL_CANARY_*` env가 생성된다 |
| `data/threshold_cycle/approvals/swing_scale_in_real_canary_YYYY-MM-DD.json` | 승인된 real swing holding의 AVG_DOWN/PYRAMID 1주 추가매수 real canary 전용 approval artifact |
...
`swing_one_share_real_canary`는 위 dry-run approval과 별도의 broker execution 품질 수집축이다. 시작 조건은 `swing_runtime_approval` hard floor와 EV trade-off 통과, 사용자 real-canary approval artifact `data/threshold_cycle/approvals/swing_one_share_real_canary_YYYY-MM-DD.json`, 장전 preopen apply, DB load/instrumentation/fallback contamination/severe downside/same-stage conflict 없음이다. phase0 guard는 `KORSTOCKSCAN_SWING_ONE_SHARE_REAL_CANARY_ENABLED=true`, 승인 code allowlist, `qty=1`, `max_new_entries_per_day=1`, `max_open_positions=3`, `max_total_notional_krw=300000`, same-symbol active real canary 1개, AVG_DOWN/PYRAMID/scale-in 실주문 금지다. `threshold_cycle_preopen_apply`는 artifact가 없으면 `one_share_real_canary_approval_artifact_missing`으로 env를 만들지 않고, artifact가 있어도 `KORSTOCKSCAN_SWING_LIVE_ORDER_DRY_RUN_ENABLED=true`를 유지한다. 이 canary의 realized PnL은 combined EV에 들어갈 수 있지만, broker receipt/order number binding/fill ratio/slippage/cancel/sell receipt는 real-only로만 판정한다. 통과해도 스윙 전체 실주문 전환은 별도 2차 계획/승인 없이는 금지한다.
