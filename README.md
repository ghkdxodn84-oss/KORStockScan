...
- threshold cycle 장후 report와 장전 apply manifest 자동화.
...
| Threshold/action weight | threshold cycle은 `manifest_only`입니다. `ThresholdOpsTransition0506` 전 live threshold runtime mutation은 금지합니다. `statistical_action_weight`와 `holding_exit_decision_matrix`는 report-only/decision-support입니다. |
...
| `src/engine/threshold_cycle_preopen_apply.py` | 장전 threshold apply manifest 생성, 현재 `manifest_only` |
...
| 자동화 | 경로 | 설명 |
| --- | --- | --- |
| threshold POSTCLOSE | `deploy/run_threshold_cycle_postclose.sh` | compact backfill과 threshold/action weight/decision matrix report 생성 |
| threshold PREOPEN | `deploy/run_threshold_cycle_preopen.sh` | `manifest_only` apply plan 생성 |
