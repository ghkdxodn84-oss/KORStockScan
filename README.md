...
| Swing lifecycle | 스윙 추천 floor는 상승장 `0.35`/하락장 `0.40`이며 fallback diagnostic 후보는 실전 후보와 분리합니다. `SWING_LIVE_ORDER_DRY_RUN_ENABLED=True`가 기본이라 장중 gatekeeper/market regime/budget/holding/exit 로직은 그대로 실행하고 실제 브로커 주문 접수만 차단합니다. `SWING_INTRADAY_LIVE_EQUIV_PROBE_ENABLED=True`이면 `blocked_swing_score_vpw`/gap/gatekeeper/regime/budget/latency block에서 observe-only virtual holding probe를 만들어 live 보유 이후 tick/exit 데이터를 수집합니다. 스윙 dry-run/probe scale-in은 `SWING_ENABLE_AVG_DOWN_SIMULATION=True`, `SWING_SCALE_IN_DYNAMIC_QTY_ENABLED=True` 기준으로 AVG_DOWN/PYRAMID 후보와 동적수량 provenance를 남기되 기본 실주문은 보내지 않습니다. 장후 `swing_lifecycle_audit`, `swing_threshold_ai_review`, `swing_improvement_automation`, `swing_runtime_approval`이 선정-진입-보유-추가매수-청산 전체 개선 후보와 EV trade-off 승인 요청을 생성합니다. 승인된 항목이 runtime env로 반영되어도 기본 dry-run과 주문 차단은 유지됩니다. |
...
| Swing scale-in real canary | `swing_scale_in_real_canary_phase0`는 별도 approval artifact가 있을 때만 승인된 real swing holding의 AVG_DOWN/PYRAMID 추가매수 1주 주문을 허용합니다. `SWING_SCALE_IN_REAL_CANARY_ENABLED=false`가 기본이고, `would_qty`/`effective_qty`는 provenance로만 남기며 실제 주문 수량은 `real_canary_actual_qty=1`로 제한합니다. sim/probe/dry-run 포지션, stale quote, OFI/QI `RISK_BEARISH`, cap 초과는 즉시 block합니다. |
...
| `data/runtime/swing_intraday_probe_state.json` | 스윙 live-equivalent intraday probe open virtual holding 재시작 복원 상태 |
...
flowchart TD
  ...
  SWINGSIM[data/report/swing_daily_simulation/swing_daily_simulation_YYYY-MM-DD.json/md]
...
### 스윙 보유 이후 AI 개입 플로우차트

스윙은 기본적으로 `SWING_LIVE_ORDER_DRY_RUN_ENABLED=True`라 실제 브로커 주문은 막고, 보유 이후 판단 경로와 provenance만 실행한다. `SWING_INTRADAY_LIVE_EQUIV_PROBE_ENABLED=True`이면 진입 전 block stage에서도 `actual_order_submitted=false`, `broker_order_forbidden=true`인 virtual holding을 만들어 같은 보유/청산 경로를 관찰한다. AI 개입은 진입 후 매 tick마다 무조건 호출되는 구조가 아니라 청산 후보, 오버나이트 후보, flow override 후보처럼 deterministic rule이 후보를 만든 시점에 제한적으로 발생한다.

```mermaid
flowchart TD
  BLOCK[WATCHING block stage<br/>score/vpw, gap, gatekeeper, regime, budget, latency]
  PROBE[swing_probe_holding_started<br/>actual_order_submitted=false]
  HOLD[SWING HOLDING<br/>swing_sim_holding_started 또는 swing_probe_holding_started]
  PRICE[실시간 가격, 일봉 추천, runtime state 갱신]
  HARD{hard/protect/order safety?}
  HARDSELL[즉시 청산 후보<br/>AI 우회]
  EXITRULE{soft stop / trailing / time stop<br/>SELL_TODAY 후보?}
  SCALE{PYRAMID / AVG_DOWN 후보?}
  MICRO[OFI/QI micro context 관찰<br/>swing_micro_* provenance]
  FLOW{holding_flow_override 적용 대상?}
  FLOWAI[AI holding-flow review<br/>action=HOLD/TRIM/EXIT]
  SMOOTH[OFI smoother postprocessor<br/>DEBOUNCE_EXIT / CONFIRM_EXIT / NO_CHANGE]
  DEFER[보류<br/>max 90초 또는 worsen +0.80%p까지]
  SELL[청산 실행 경로<br/>dry-run이면 actual_order_submitted=false]
  ADD[추가매수 관찰 경로<br/>swing_sim_scale_in_* 또는 blocked]
  OVER[15:20 오버나이트 SELL_TODAY 후보]
  OVERAI[AI overnight review<br/>HOLD_OVERNIGHT / SELL_TODAY]
  REPORT[swing_lifecycle_audit<br/>swing_improvement_automation<br/>swing_runtime_approval]

  BLOCK -->|probe cap pass| PROBE --> HOLD
  HOLD --> PRICE
  PRICE --> HARD
  HARD -->|yes| HARDSELL --> SELL
  HARD -->|no| EXITRULE
  EXITRULE -->|no| SCALE
  SCALE -->|candidate| MICRO --> ADD --> REPORT
  SCALE -->|none| OVER
  EXITRULE -->|candidate| FLOW
  FLOW -->|yes| FLOWAI --> SMOOTH
  FLOW -->|no| SELL
  SMOOTH -->|HOLD/TRIM or debounce| DEFER --> PRICE
  SMOOTH -->|EXIT or confirm| SELL
  OVER -->|SELL_TODAY candidate| OVERAI
  OVERAI -->|HOLD_OVERNIGHT| DEFER
  OVERAI -->|SELL_TODAY/TRIM| SELL
  SELL --> REPORT
```

### 스캘핑 보유 이후 AI 개입 플로우차트

스캘핑은 보유 중 deterministic stop/trailing/scale-in guard가 먼저 후보를 만들고, AI는 후보의 품질 확인 또는 flow override에 개입한다. hard stop, protect hard stop, 주문/잔고 safety는 AI보다 우선한다.

```mermaid
flowchart TD
  HOLD[SCALPING HOLDING<br/>real 또는 scalp_ai_buy_all sim]
  TICK[WS tick, orderbook, 체결강도, MFE/MAE 갱신]
  HARD{hard stop / protect hard / receipt safety?}
  HARDSELL[즉시 매도<br/>AI 우회]
  PRESET{preset TP touch?}
  PRESETAI[+0.8% preset TP AI 1회 리뷰<br/>EXIT 또는 HOLD]
  SOFT{soft stop / trailing / bad-entry<br/>청산 후보?}
  ADD{REVERSAL_ADD / PYRAMID 후보?}
  PRICEQ[scale-in price resolver + dynamic qty<br/>P2는 observe-only]
  FLOW{holding_flow_override 적용 대상?}
  FLOWAI[AI holding-flow review<br/>최근 tick 30개 + 분봉 60개]
  OFI[OFI smoother postprocessor<br/>DEBOUNCE_EXIT / CONFIRM_EXIT / NO_CHANGE]
  DEFER[보류<br/>max 90초 또는 worsen +0.80%p]
  SELL[매도 주문 또는 sim assumed sell<br/>exit_decision_source 기록]
  ADDORDER[추가매수 주문 또는 sim assumed fill<br/>actual_order_submitted 분리]
  OVER[15:20 overnight SELL_TODAY 후보]
  OVERAI[AI overnight review<br/>HOLD_OVERNIGHT / SELL_TODAY]
  REPORT[holding_exit_observation<br/>SAW / threshold-cycle / daily EV]
  ...
  SOFT -->|candidate| FLOW
  SOFT -->|none| ADD
  ADD -->|candidate| PRICEQ --> ADDORDER --> REPORT
  ADD -->|none| OVER
  FLOW -->|yes| FLOWAI --> OFI
  FLOW -->|no| SELL
  OFI -->|HOLD/TRIM or debounce| DEFER --> TICK
  OFI -->|EXIT or confirm| SELL
  OVER -->|SELL_TODAY candidate| OVERAI
  OVERAI -->|HOLD_OVERNIGHT| TICK
  OVERAI -->|SELL_TODAY/TRIM| SELL
  SELL --> REPORT
```
...
| Swing scale-in real canary | 별도 승인 필요 | `swing_scale_in_real_canary_YYYY-MM-DD.json` approval artifact가 있어야만 `PYRAMID`/`AVG_DOWN` 중 기준 통과 arm의 1주 추가매수 실주문을 허용한다. initial BUY/SELL real canary와 서로 자동 승인하지 않는다. |
