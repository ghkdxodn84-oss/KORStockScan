...
현재 기준 문서는 [Plan Rebase](docs/plan-korStockScanPerformanceOptimization.rebase.md)이며, 실제 실행 항목은 날짜별 `docs/YYYY-MM-DD-stage2-todo-checklist.md`가 소유합니다. `2026-05-09` checklist는 없고, 최신 미래 실행표는 [2026-05-11 checklist](docs/2026-05-11-stage2-todo-checklist.md)입니다.
...
- 스윙 추천 생성, 선정 funnel, live 주문 dry-run, lifecycle audit, threshold AI proposal review, improvement workorder 자동화.
...
| Position sizing | 신규 BUY, REVERSAL_ADD/AVG_DOWN, PYRAMID의 1주 수량 cap은 2026-05-09 사용자 지시로 기본 OFF입니다. 수량은 기존 예산 cap, `SCALPING_MAX_BUY_BUDGET_KRW`, `MAX_POSITION_PCT`, 가격 resolver, pending/cooldown/protection guard 안에서만 산출합니다. |
| Swing lifecycle | 스윙 추천 floor는 상승장 `0.35`/하락장 `0.40`이며 fallback diagnostic 후보는 실전 후보와 분리합니다. `SWING_LIVE_ORDER_DRY_RUN_ENABLED=True`가 기본이라 장중 gatekeeper/market regime/budget/holding/exit 로직은 그대로 실행하고 실제 브로커 주문 접수만 차단합니다. 장후 `swing_lifecycle_audit`, `swing_threshold_ai_review`, `swing_improvement_automation`이 선정-진입-보유-추가매수-청산 전체 개선 후보를 생성합니다. |
...
| `src/engine/swing_lifecycle_audit.py` | 스윙 선정-진입-보유-추가매수-청산 lifecycle audit, threshold AI review, improvement automation |
...
현재 daily threshold/scalping/swing improvement 체인은 사람의 장전 승인 없이 돌아간다. 사람의 개입은 Project/Calendar 동기화, 신규 code improvement order를 실제 코드 패치로 전환하는 별도 작업, 운영 장애 대응에만 남긴다.
...
| Swing lifecycle 결과의 runtime 직접 반영 | 차단 | `swing_lifecycle_audit`와 `swing_improvement_automation`은 선정-진입-보유-추가매수-청산 관찰축과 workorder 후보만 만든다. 실제 스윙 live 변경은 생성된 workorder를 사람이 Codex에 넣어 별도 구현 지시할 때만 진행한다. |
