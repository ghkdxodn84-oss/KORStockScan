...
# 1. 현재 이 파일(constants.py)의 위치: KORStockScan/src/utils
UTILS_DIR = os.path.dirname(os.path.abspath(__file__))

# 2. 소스코드 루트 (UTILS_DIR에서 한 칸 위): KORStockScan/src
SRC_DIR = os.path.dirname(UTILS_DIR)

# 3. 프로젝트 최상위 루트 (SRC_DIR에서 한 칸 위): KORStockScan
PROJECT_ROOT = os.path.dirname(SRC_DIR)

# 4. 이제 여기서 모든 공통 경로를 뚫어줍니다!
DATA_DIR = os.path.join(PROJECT_ROOT, 'data')
LOGS_DIR = os.path.join(PROJECT_ROOT, 'logs')
NOTIFY_DIR = os.path.join(PROJECT_ROOT, 'notify')
...
TRADING_RULES = {
    ...
    'INVEST_RATIO_SCALPING': 0.20,   # 3. 초단타 스캘핑 (20% - AI의 빠른 판단에 따라 적극적으로 매매)
    ...
    # 🎯 유저권한별 기능 제한 설정값
    # ==========================================
    'VIP_LIQUIDITY_THRESHOLD': 500_000_000  # VIP 전용 호가 잔량 대금 기준 (5억)
}