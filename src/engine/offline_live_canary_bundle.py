...
def _copy_file(src: Path, dest_root: Path, copied: list[dict[str, object]]) -> None:
    rel = _bundle_rel(src)
    dest = dest_root / rel
...
def _copy_optional_file(src: Path, dest_root: Path, copied: list[dict[str, object]], missing: list[str]) -> None:
    if src.exists():
        _copy_file(src, dest_root, copied)
    else:
...
def export_bundle(
    *,
    ...
    _copy_glob(POST_SELL_DIR, "post_sell_candidates_*.jsonl", root, copied_files, missing_files)
    _copy_glob(POST_SELL_DIR, "post_sell_evaluations_*.jsonl", root, copied_files, missing_files)
    ...
    _copy_optional_file(GATEKEEPER_DIR / f"gatekeeper_snapshots_{target_date}.jsonl", root, copied_files, missing_files)
