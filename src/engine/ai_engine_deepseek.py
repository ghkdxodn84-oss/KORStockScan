...
class DeepSeekSniperEngine:
    ...
    def _record_failure_and_maybe_disable(self, *, context_name):
        ...
        if failure_count >= self.max_consecutive_failures:
            ...
            log_error(
                f"🚨 DeepSeek 엔진 비활성화 (연속 실패 {failure_count}회 초과, "
...
{str(input_data_text)}"""
    ...
    def evaluate_scalping_entry_price(
        ...
        try:
            ...
            log_error(f"🚨 [ENTRY_PRICE] DeepSeek 가격결정 에러 ({stock_name}, 연속 실패 {failure_count}회): {e}")
            return self._annotate_analysis_result(
                normalize_scalping_entry_price_result(
...
"""
    ...
    def evaluate_scalping_holding_flow(
        ...
        try:
            ...
            log_error(f"🚨 [HOLDING_FLOW] DeepSeek 판정 에러 ({stock_name}/{decision_kind}, 연속 실패 {failure_count}회): {e}")
            return self._annotate_analysis_result(
                {
                    "action": "EXIT",
                    "score": 0,
    ...
    def evaluate_scalping_overnight_decision(self, stock_name, stock_code, realtime_ctx):
        ...
        try:
            ...
            log_error(f"🚨 [SCALPING 오버나이트 판정] DeepSeek 에러 ({stock_name}, 연속 실패 {failure_count}회): {e}")
            return self._annotate_analysis_result(
                {
                    'action': 'SELL_TODAY',
                    'confidence': 0,
